...
public class Client {
    ...
    public static void checkNetworkSpeedSpecificIp() throws Exception {
        ...
        checkNetworkSpeed(InetAddress.getByName(ip), FileServer.PORT);
    }
    ...
    public static void scanIpAddress() {
        ...
        for (int i = 1; i < 65535; i++) {
            try {
                ...
                System.out.println("There is a main.java.server on port " + i + " of " + host);
            } catch (UnknownHostException ex) {
                Logger.log(ex);
                Logger.log("Not a valid address entered");
                System.exit(1);
            } catch (IOException ex) {
                // must not be a main.java.server on this port
            }
        }
    }
